...
dependencies: mlr (miller), datamash
"""
...
def usage(message=None):
    ...
    if message is not None:
        ...
        sys.stderr.write("\n")
    usage_message = """Usage: dump_merge_watchlist.py --wiki <wikiname> --configfile <path>
...
class MergeAdder():
    ...
    def merge(self, merge_into, to_merge, merged_path, verbose, dryrun):
        ...
        '''
        command = "{sort} -t$'\t' -k1n -k2  -m --parallel=1 ".format(sort=self.wiki.config.sort)
        command += '<({gzip} -dc {merge_into}) <({gzip} -dc {to_merge}) '.format(
            gzip=self.wiki.config.gzip, merge_into=merge_into, to_merge=to_merge)

        command += " | {datamash} -g 1,2 sum 3 ".format(datamash=self.wiki.config.datamash)
...
def do_main():
    ...
    wiki.config.datamash = '/usr/bin/datamash'
